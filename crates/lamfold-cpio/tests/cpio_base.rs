//! Integration test against a real `cpio -H newc` archive.
//!
//! Fixture `fixtures/initramfs.newc` (built with `find -printf '%P' | cpio -o -H newc`):
//!   readme.txt    "romfs root file\n"  (16 B)   [shared tree from the romfs fixture]
//!   sub/inner.txt "nested file\n"       (12 B)
//!   big.dat       byte[i] = i % 256     (300000 B)
//!   link.txt   → readme.txt             (symlink)

use lamfold::{BlockCache, FileKind, FoldFrontend, NoVerifier, SliceSource, SubstrateCtx};
use lamfold_cpio::Cpio;

const ARCHIVE: &[u8] = include_bytes!("fixtures/initramfs.newc");

#[test]
fn probe_accepts_newc() {
    let mut src = SliceSource::new(ARCHIVE);
    assert!(Cpio::probe(&mut src).unwrap());
}

#[test]
fn probe_rejects_non_cpio() {
...
#[test]
fn builds_tree_reads_files_and_symlink() {
    ...
    let mut fs = Cpio::open(SliceSource::new(ARCHIVE), &mut cx).unwrap();
    let root = fs.root();
    ...
    // Directory implied by the nested path "sub/inner.txt".
